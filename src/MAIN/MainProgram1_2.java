...
import util.History1_0;
import util.SQL_DDL1_0;
import util.StdFiles1_0;
import util.Utility;
...
public class MainProgram1_2 {
	...
	public static void main(String[] args) throws IOException, ParseException, SQLException, InterruptedException {
        ...
        boolean dataFilesExist = History1_0.checkStandardFiles(dir);
	...
	public static void printWelcomeMessage() {
		...
		System.out.println("TAGOPSIN - TAxonomy, Gene, Ontology, Protein, Structure INtegrated\n"
				+ "version 1.2");
		System.out.println("Bundhoo et al. (2020), Journal, vv(ii):pp-pp, doi: xxx");
	...
	public static void create_db_relations() {
		...
		try {
			...
			output.format("%s", SQL_DDL1_0.getSQL());
			output.close();
	...
	public static String getDataDirFromHistory() throws IOException {
		...
		boolean hasHistory = History1_0.historyExists();
	...
	public static void downloadStandardFiles(String dir) throws IOException {
		...
		if (! History1_0.checkTaxonomyFile(dir))
		    
			StdFiles1_0.downloadTaxonomy(dir);
		
		if (! History1_0.checkUniProtFiles(dir))
			
			StdFiles1_0.downloadUniProt(dir);
		
		if (! History1_0.checkGOfile(dir))
			
			StdFiles1_0.downloadGO(dir);
		
		if (! History1_0.checkPfamFile(dir))
			
			StdFiles1_0.downloadPfam(dir);
		
		if (! History1_0.checkPDBfiles(dir))
			
			StdFiles1_0.downloadPDB(dir);
	...
	public static void downloadAndOrParseByOrganismFile(String fName, String type, String organism) throws IOException {
		...
		if (! History1_0.dataFileIsPresent(dir+"/uniprot/"+fName.substring(0, fName.length()-3))) {
	...
	public static void downloadAndOrParseDefaultIdMappingFile(String type, String organism) throws IOException {
		...
		if (! History1_0.dataFileIsPresent(dir+"/uniprot/idmapping.dat")) {
