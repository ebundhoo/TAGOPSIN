...
/**
 * This class contains all the functions used by the main program of TAGOPSIN (MainProgram.java) to 
 * download standard data files.
 * 
 * This version adds the file "uniprot_sprot_varsplic.fasta.gz" found on UniProt FTP
 * 
...
public class StdFiles {
	...
	public static void downloadUniProt(String dir) throws IOException {
		...
		//Download uniprot_sprot_varsplic.fasta.gz from UniProt FTP and decompress
		System.out.println("Downloading "
				+ "ftp://ftp.uniprot.org/pub/databases/uniprot/current_release/knowledgebase/complete/uniprot_sprot_varsplic.fasta.gz");
		
		url = new URL("ftp://ftp.uniprot.org/pub/databases/uniprot/current_release/knowledgebase/complete/uniprot_sprot_varsplic.fasta.gz");
		
		FTP.downloadFile(url, dir+"/uniprot/uniprot_sprot_varsplic.fasta.gz");
		
		System.out.println("Decompressing uniprot_sprot_varsplic.fasta.gz...");
		try {
			Process proc = rt.exec("gunzip -d " + dir + "/uniprot/uniprot_sprot_varsplic.fasta.gz");
			proc.waitFor();
		} catch (InterruptedException e) {
			System.out.println("Process interrupted. Terminating now...");
			System.exit(1);
